...
class Grid:
    ...
    def print_final_iteration_table(self, decimals=6):
        if self.final_grid is None:
            print("Решение еще не выполнено!")
            return
        
        # Создаем заголовки для таблицы
        headers = ["Y\\X"] + [f"{i * self.h:.2f}" for i in range(self.n + 1)]
        
        # Подготавливаем данные для таблицы
        table_data = []
        for j in range(self.m + 1):
            row = [f"{j * self.k:.2f}"] + [f"{self.final_grid[j][i]:.{decimals}f}" for i in range(self.n + 1)]
...
if __name__ == '__main__':
    ...
    grid.print_final_iteration_table(decimals=4)  # Можно изменить количество знаков после запятой
